...
const (
	// DefaultTargetChunkSize is the fallback number of OptimizedTarget entries per stream
	// message, used only if chunkTargets is called with a non-positive chunk size.
	DefaultTargetChunkSize = 250

	// DefaultChangedTargetChunkSize is the fallback number of ChangedTarget entries per
	// stream message. A ChangedTarget carries both old_target and new_target (2× an
	// OptimizedTarget), so it gets half the plain-target fallback.
	DefaultChangedTargetChunkSize = 125

	// DefaultMetadataMapChunkSize is the fallback max entries per metadata message chunk,
	// used only if ChunkMetadata is called with a non-positive chunk size.
