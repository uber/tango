...
func TestNewGitRequest_InvalidPath(t *testing.T) {
	req := NewGitRequest(nil, "invalid", "git@github.com:org/repo.git", "baseRef", "", zap.NewNop().Sugar())
	require.NotNil(t, req)
}

func TestNewGitRequest_ExtractsID(t *testing.T) {
	r := NewGitRequest(nil, "/org/repo/pull/456", "git@github.com:org/repo.git", "baseRef", "abc123", zap.NewNop().Sugar())
	gr, ok := r.(*gitRequest)
...
func TestGitRequest_Apply_CommitIsAncestor_Success(t *testing.T) {
	...
	req := NewGitRequest(git, "123", "git@github.com:org/repo.git", "baseRef", "deadbeef", zap.NewNop().Sugar())
...
func TestGitRequest_Apply_CommitNotAncestor_ReturnsError(t *testing.T) {
	...
	req := NewGitRequest(git, "456", "git@github.com:org/repo.git", "baseRef", "deadbeef", zap.NewNop().Sugar())
	err := req.Apply(context.Background())
	require.Error(t, err)
...
func TestGitRequest_Apply_IsAncestorFails_ReturnsError(t *testing.T) {
	...
	req := NewGitRequest(git, "789", "git@github.com:org/repo.git", "baseRef", "deadbeef", zap.NewNop().Sugar())
	err := req.Apply(context.Background())
	require.Error(t, err)
