...
func TestCompareTargetGraphs_UnspecifiedWhenNoSourceFileRuleKnown(t *testing.T) {
	c := &controller{logger: zaptest.NewLogger(t)}
	...
	require.NotNil(t, cs)
	require.Len(t, cs.GetChangedTargets(), 1)
	require.Equal(t, pb.CHANGE_TYPE_UNSPECIFIED, cs.GetChangedTargets()[0].GetChangeType())
}
