...
// SplitMetadata splits the metadata maps into multiple GraphMetadata
...
// entry wire size; consumers merge all metadata messages before use. The
// small maps (rule_type, tag, attribute_name) are sent in the first
// message. Always returns at least one message. Returns an error if a
// non-first message is completely empty.
func SplitMetadata(
	...
	maxBytes int,
) ([]*entity.GraphMetadata, error) {
	targetGroups := splitMapByBytes(targetIDToName, maxBytes)
	attrValGroups := splitMapByBytes(attrStrValIDToVal, maxBytes)

	metas := make([]*entity.GraphMetadata, 0, max(1, len(targetGroups)+len(attrValGroups)))
	for _, g := range targetGroups {
		metas = append(metas, &entity.GraphMetadata{TargetIDMapping: g})
	}
	for _, g := range attrValGroups {
		metas = append(metas, &entity.GraphMetadata{AttributeStringValueMapping: g})
	}
	if len(metas) == 0 {
		metas = append(metas, &entity.GraphMetadata{})
	}
...
// MapEntryWireSize returns the serialized size of one map[int32]string
// entry, mirroring the protobuf Metadata.Size() computation exactly.
